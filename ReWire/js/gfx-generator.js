...
const v01 = { x: 0, y: 1 };
const v11 = { x: 1, y: 1 };
const n21 = (v) => ((Math.sin(v.x * 100 + v.y * 6574) + 1) * 564) % 1;
...
const newCol = (r = 1, g = 1, b = 1, a = 1) => ({ r, g, b, a });
const mulCol = (color, v) => ({
    r: color.r * v,
    g: color.g * v,
    b: color.b * v,
    a: color.a
...
const createIsolatorSprite = (size) => {
    const sw = 4 / size;
...
const createGear = (px, py, outerSize, innerSize, step) => {
...
const createLedSprite = () => generateImage(21, 21, v => {
    const cv = subV(v, halfV);
...
const createGlow = (color) => generateImage(80, 80, v => {
    const cv = subV(v, halfV);
    const d = 1 - lenV(cv) * 2;
...
const createMetal = (a, d) => {
    return 0.9 + 0.1 * Math.sin(a * 6) * 0.9 + 0.1 * Math.sin(a * 4)
        - (noise({ x: (a + 4 + d * 5) * 2, y: d * 80 }) * 0.1);
};

const createRingGlow = (color) => generateImage(62, 62, v => {
    const cv = subV(v, halfV);
    const d = 1 - lenV(cv) * 2;
...
const createConnectorButtons = (lightColor, size) => {
    const shadowBlur = 0.2;
    const shadowDistance = 0.04;
    const shadowScale = 1.1;
