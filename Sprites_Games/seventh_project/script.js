window.addEventListener("load", function () {
  ...
  class Player {
    ...
    //rescale the width for the collision for the mobile support
    update(input, deltaTime, enemies) {
      //collision detection:
      enemies.forEach((enemy) => {
        const dx = (enemy.x + enemy.width/2 - 20) - (this.x + this.width/2); //--here substract from the width, horizontal axis.
        const dy = (enemy.y + enemy.height/2) - (this.y + this.height/2 + 20); //--here add to height
        const distance = Math.sqrt(dx * dx + dy * dy);
        if (distance < enemy.width / 3 + this.width / 3) { //--here too
          gameOver = true;
        }
      });
      // sprite animations
